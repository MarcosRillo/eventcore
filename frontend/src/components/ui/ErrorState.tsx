...
const ErrorState = ({
  ...
  const variantClasses: Record<string, { container: string; iconSize: string }> = {
    ...
    card: {
      container: 'py-8 px-6 bg-red-50/50 border border-red-100 rounded-lg',
      iconSize: 'w-10 h-10',
    },
  }
  ...
  return (
    <div
      ...
      role="alert"
    >
      <div className={`text-red-400 mb-4 ${styles.iconSize}`}>
        {icon || defaultIcon}
      </div>
      ...
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="mt-4 inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-red-600 bg-red-50 hover:bg-red-100 rounded-md transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-red-500/40"
        >
